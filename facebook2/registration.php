<?php
  ...
  if(isset($data['do_sign'])) {
    ...
    if($stmt) {
      ...
      /* связываем параметры с метками */
      mysqli_stmt_bind_param($stmt, "sss", $name, $email, $encryptedPassword);
      /* запускаем запрос */
      ...
      if($success) {
        echo '<div style="color: green; font-weight: bold;">Регистрация прошла успешно! <a href="login.php">Авторизуйтесь</a>, пожалуйста!</div><hr/>';
          $CreateSql = "INSERT INTO `user` (`name`, `surname` ,`gender`) VALUES ('$firstname', '$surname', '$gender');";
          mysqli_query($link, $CreateSql) or die(mysqli_error($link));
          $CreateSql = "INSERT INTO access (login, email, password) VALUES ('$name','$email','$password');";
          mysqli_query($link, $CreateSql) or die(mysqli_error($link));
      } else {
